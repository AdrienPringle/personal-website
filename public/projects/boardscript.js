...
let boardSize = 10;
let blockSize = cvs.width/(boardSize+1);
...
for(let y = 0; y < boardSize; y++){
  ...
  document.body.insertBefore(document.createElement('BR'), document.getElementById('reset'));
}
...
function nextTurn(){
  ...
  for(var i = 0; i < 2; i++){
    ...
    if(plays[i].totalMoves > 6){plays[i].totalMoves = 6;}
...
function checkMoves(playerNumber, move){
  for(var i = 0; moves[playerNumber][i] != undefined; i++){
    if(moves[playerNumber][i].x == move.x && moves[playerNumber][i].y == move.y){
      return true;
    }
  }
  return false;
...
function checkTiles(tx,ty){
  ...
  for(let x = -1; x < 2; x++){
    for(let y = -1; y < 2; y++){
      try{
        if(board[x+tx][y+ty].player != 0 && (x != 0 || y != 0)){adjacent++;}
      }catch(err){}
    }
  }
  return adjacent;
...
function drawBoard(){
  ...
  for(let i = 0; i < moves[player-1].length; i++){
    if(moves[player-1][i] != undefined){
      colorRect(moves[player-1][i].x*blockSize, moves[player-1][i].y*blockSize, blockSize, blockSize, playerColor[player]);
    }
  }
  drawNumbers();
}
...
function drawNumbers(){
  ...
  for(let x = 0; x < boardSize; x++){
    for(let y = 0; y < boardSize; y++){
      if(board[x][y].player != 0){
  		   cxt.fillText(board[x][y].number, x*blockSize + (blockSize/2 - 10), y*blockSize + (blockSize/2 + 13));
      }
    }
  }
}
  ...
  for(let x = 0; x < boardSize; x++){
    for(let y = 0; y < boardSize; y++){
      if((x+y)%2 == 0){
        ...
        colorRect(x*blockSize, y*blockSize, blockSize, blockSize, 'rgb(200,200,200)');
      }

      let tilePlayer = board[x][y].player;
      if(tilePlayer != 0){
        colorRect(x*blockSize, y*blockSize, blockSize, blockSize, playerColor[tilePlayer]);
      }
...
function checkPendingMoves(mousePos){
  for(let i = 0; i < moves[player-1].length; i++){
    if(moves[player-1][i] != undefined){
      if(mousePos.x == moves[player-1][i].x && mousePos.y == moves[player-1][i].y){
...
function addPendingMove(mousePos){
  if(!checkPendingMoves(mousePos) && plays[player-1].currentMove != moves[player-1].length){
...
window.onload = function(){
  ...
  //drawBoard();
  document.getElementById('turn').addEventListener('click',function(evt){handleNextTurn(evt); drawBoard();})
  document.getElementById('reset').addEventListener('click',function(evt){init();})
}
